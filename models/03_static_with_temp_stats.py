...
X_static = pd.read_pickle(...).drop(columns=[*original_feat, 'bpd_label'])

scaler = StandardScaler()
X_static_scaled = scaler.fit_transform(X_static)

pca = PCA()
X_pca = pca.fit_transform(X_static_scaled)

explained_variance_ratio = pca.explained_variance_ratio_
cumsum = np.cumsum(explained_variance_ratio)
n_components = np.argmax(cumsum >= .95) + 1

components = pca.components_[:n_components]
feat_contributions = np.sum(np.abs(components), axis=0)

feature_names = X_static.columns
contributions_df = pd.DataFrame(feat_contributions, index=feature_names, columns=['Contribution'])
top_features = contributions_df.nlargest(n_components, 'Contribution')


data = pd.read_pickle(...)
data = data[[*top_features.index.to_list(), *original_feat, 'bpd_label']]
...
grid_search = GridSearchCV(estimator=BaselineModel(input_dim=n_components + 6), param_grid=param_grid, scoring='f1', cv=cv, verbose=3)
